...
def read_mda(filename):
    ...
    metadata["PVs"] = {
        as_str(key): dict(
            ...
            value=values[2],
            EPICS_type=mda.EPICS_types_dict.get(
    ...
    for scan in mda_obj[1:]:
        ...
        # print(f"{scan.rank=}")
        content[f"{prefix}time"] = as_str(scan.time)  # TODO: convert to timestamp (must assume a time zone)
